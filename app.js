...
(() => {
  ...
  const defaults={baseRate:44.37,wdMult:1,satMult:1.5,sunMult:2,otMult:1.8,hdRate:4.87,maRate:4.87,nightRate:5.77,lease:546.95,gesb:100,postTax:7.40,calibration:87};
  ...
  const opts=selected=>['',...Object.keys(SHIFT_DATA).sort((a,b)=>a.localeCompare(b,undefined,{numeric:true}))].map(code=>`<option value="${code}" ${code===selected?'selected':''}>${code?`${code} — ${SHIFT_DATA[code].name}`:'Off / no shift'}</option>`).join('');
  ...
  function applyShiftDefaults(card,date,force=false){
    ...
    label.textContent=`${data.name} • ${start.value}–${finish.value}`;
  }
  ...
  function recalculate(){
    ...
    latestResult.dayTotals.forEach((row,i)=>{
      ...
      if(row.start&&row.finish&&SHIFT_DATA[current.days[i].code]) card.querySelector('.shift-time').textContent=`${SHIFT_DATA[current.days[i].code].name} • ${row.start}–${row.finish}`;
    });
    $('#gross').textContent=money(latestResult.gross);
