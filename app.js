...
function populateAgreementControls(){
  const aSel=$('#agreementId'),cSel=$('#classificationId'),rate=$('#classificationRate');
  if(!aSel||!cSel||!rate)return;
  ...
  rate.value=current.settings.classificationRate;
}
function applyAgreementPreset(){
  ...
  current.settings.baseRate=current.settings.classificationRate;
  if($('#classificationRate'))$('#classificationRate').value=current.settings.classificationRate;
  if($('#baseRate'))$('#baseRate').value=current.settings.classificationRate;
  recalculate();
}
...
function readSettings(){
  current.settings.agreementId=$('#agreementId')?.value||current.settings.agreementId||'custom';
  current.settings.classificationId=$('#classificationId')?.value||current.settings.classificationId||'custom';
  current.settings.classificationRate=Number($('#classificationRate')?.value??current.settings.classificationRate)||0;
