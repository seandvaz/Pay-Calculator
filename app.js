...
(() => {
  ...
  const rosterVisualKind=entry=>{
    ...
    const row=entry.row;
    const entered=row?.entered ?? Boolean(row?.code);
  ...
  function rosterDisplayCode(entry){
    if(!entry)return'—';
    const entered=entry.row?.entered ?? Boolean(entry.row?.code);
    if(!entered)return'—';
  ...
  function renderCalendar(selectedKey=''){
    ...
    const first=mondayOfRosterWeek(firstOfMonth);
    const last=addRosterDays(mondayOfRosterWeek(lastOfMonth),6);
    ...
    for(let date=new Date(first);date<=last;date=addRosterDays(date,1)){
      ...
      button.innerHTML=`<span class="calendar-date">${date.getDate()}</span>
        <strong>${rosterDisplayCode(entry)}</strong>
  ...
  const asCycleRecord=(cycle,id='')=>{
    if(!cycle)return null;
    const result=cycle===current&&latestResult?latestResult:PayCalc.calculate(cycle);
    return {
      ...cycle,
      id:cycle.id||id||`cycle-${cycle.startDate}`,
      summary:cycle.summary||{
        gross:result.gross,taxable:result.taxable,tax:result.tax,
        hours:result.hours,net:result.net,netHourly:result.netHourly
      }
    };
  };

  function availablePayCycles(){
    const saved=AppStorage.loadCycles().map(c=>asCycleRecord(c));
    const same=saved.some(c=>c.startDate===current.startDate);
    if(!same && current?.startDate){
      saved.push(asCycleRecord(current,'current-unsaved'));
    }
    return saved;
  }
  ...
  function upcomingPayCycle(){
    ...
    const upcoming=availablePayCycles()
      .filter(c=>paydayFor(c.startDate)>=today)
      .sort((a,b)=>paydayFor(a.startDate)-paydayFor(b.startDate));
    if(upcoming.length)return upcoming[0];
    return availablePayCycles().sort((a,b)=>b.startDate.localeCompare(a.startDate))[0]||asCycleRecord(current,'current-unsaved');
  }

  function resultForCycle(cycle){
    if(!cycle)return null;
  ...
  function renderFinancialYearSummary(referenceCycle){
    if(!$('#homeFyLabel'))return;
    const refDate=referenceCycle?.startDate?parseDate(referenceCycle.startDate):new Date();
    const fy=financialYearFor(refDate);
    ...
    $('#homeCycleCount').textContent=futureCount
      ?`${cycles.length} ${cycles.length===1?'pay':'pays'} • ${futureCount} future`
      :`${cycles.length} ${cycles.length===1?'pay':'pays'}`;
    ...
    const diff=$('#homeFyDifference');
    diff.textContent=money(totals.difference);
    diff.className=totals.difference>0?'difference-positive':totals.difference<0?'difference-negative':'difference-neutral';
    ...
    }else lastActualRow.hidden=true;

    const next=upcomingPayCycle();
    $('#homePayday').textContent=next
      ?paydayFor(next.startDate).toLocaleDateString('en-AU',{day:'numeric',month:'short',year:'numeric'})
      :'—';
  }

  function renderPayResult(cycle){
    if(!cycle)return;
    const result=resultForCycle(cycle);
    if(!result)return;
    $('#payRange').textContent=rangeLabel(cycle.startDate);
    $('#payPayday').textContent=`Payday ${paydayFor(cycle.startDate).toLocaleDateString('en-AU',{day:'numeric',month:'short',year:'numeric'})}`;
    const status=smartStatus(cycle);
    $('#payStatusLabel').textContent=statusLabel(status);
    $('#workedPay').textContent=money(result.breakdown.workedPay);
    $('#publicHolidayPay').textContent=money(result.breakdown.publicHolidayPay);
    renderPublicHolidayBreakdown(result.publicHolidayDetails);
    $('#annualLeavePay').textContent=money(result.breakdown.annualLeavePay);
    $('#sickLeavePay').textContent=money(result.breakdown.sickLeavePay);
    $('#lslPay').textContent=money(result.breakdown.lslPay);
    $('#lwopPay').textContent=money(result.breakdown.lwopPay);
    $('#extrasPay').textContent=money(result.breakdown.extrasPay);
    renderAllowanceBreakdown(result.allowanceDetails,cycle.startDate);
    $('#additionalHoursPay').textContent=money(result.breakdown.additionalHoursPay);
    $('#gross').textContent=money(result.gross);
    $('#taxable').textContent=money(result.taxable);
    $('#tax').textContent=money(result.tax);
    $('#hours').textContent=result.hours.toFixed(1);
    $('#net').textContent=money(result.net);
    $('#netHourly').textContent=money(result.netHourly);
    renderFinancialYearSummary(cycle);
    const upcoming=upcomingPayCycle();
    $('#backToUpcomingPay').hidden=!upcoming||cycle.startDate===upcoming.startDate;
  }

  function paySection(title,cycles,container){
    if(!cycles.length)return;
    const section=document.createElement('section');
    section.className='pay-history-section';
    section.innerHTML=`<h3>${title}</h3>`;
    cycles.forEach(c=>section.appendChild(makePayCycleCard(c)));
    container.appendChild(section);
  }

  function makePayCycleCard(c){
    const actual=c.actualDeposit!==''&&c.actualDeposit!=null?Number(c.actualDeposit):null;
    const diff=actual==null?null:actual-(Number(c.summary?.net)||0);
    const status=smartStatus(c),payday=paydayFor(c.startDate);
    const card=document.createElement('div');
    card.className='saved-card compact-saved';
    ...
    <div class="saved-card-collapse">
      ...
      <div class="saved-meta">
        ...
        <div><span>Hours</span><b>${Number(c.summary?.hours||0).toFixed(1)}</b></div>
      ...
      <div class="quick-actual-entry">
        <label>Actual deposit
          <input class="quick-actual-input" type="number" step="0.01" inputmode="decimal" placeholder="${actual==null?'Enter amount':actual}" value="${actual==null?'':actual}">
        </label>
      ...
      <div class="saved-actions">
        <button class="view-pay" type="button">View pay</button>
        <button class="edit-pay-roster" type="button">Open roster</button>
        ${c.id==='current-unsaved'?'':'<button class="delete-saved" type="button">Delete</button>'}
      </div>
    </div>`;
    card.querySelector('.saved-card-toggle').onclick=()=>card.classList.toggle('open');
    card.querySelector('.view-pay').onclick=()=>{
      selectedPayCycleId=c.id;
      renderPayResult(c);
      window.scrollTo({top:0,behavior:'smooth'});
    };
    card.querySelector('.edit-pay-roster').onclick=()=>{
      current=JSON.parse(JSON.stringify({startDate:c.startDate,settings:c.settings,days:c.days}));
      current.days=current.days.map(row=>({...emptyDay(),...row,entered:row.entered??Boolean(row.code)}));
      AppStorage.saveCurrent(current);
      buildRoster();
      go('roster');
      toast('Pay cycle opened in roster');
    };
    const saveActual=card.querySelector('.save-quick-actual');
    if(saveActual)saveActual.onclick=()=>{
      if(c.id==='current-unsaved'){toast('Save the pay cycle first');return}
      const input=card.querySelector('.quick-actual-input'),value=input.value.trim();
      if(value===''){toast('Enter the actual deposit');input.focus();return}
      const saved=AppStorage.loadCycles(),i=saved.findIndex(x=>x.id===c.id);if(i<0)return;
      saved[i].actualDeposit=value;saved[i].updatedAt=new Date().toISOString();AppStorage.saveCycles(saved);
      renderPayScreen();renderHomeDashboard();toast('Actual deposit saved');
    };
    const clear=card.querySelector('.clear-quick-actual');
    if(clear)clear.onclick=()=>{
      if(!confirm('Clear the actual deposit for this pay?'))return;
      const saved=AppStorage.loadCycles(),i=saved.findIndex(x=>x.id===c.id);if(i<0)return;
      saved[i].actualDeposit='';saved[i].updatedAt=new Date().toISOString();AppStorage.saveCycles(saved);
      renderPayScreen();renderHomeDashboard();toast('Actual deposit cleared');
    };
    const del=card.querySelector('.delete-saved');
    if(del)del.onclick=()=>{
      if(!confirm('Delete this saved pay cycle?'))return;
      AppStorage.saveCycles(AppStorage.loadCycles().filter(x=>x.id!==c.id));
      renderPayScreen();renderHomeDashboard();toast('Deleted');
    };
    return card;
  }

  function renderPayCycleList(){
    const wrap=$('#payCycleList');if(!wrap)return;
    wrap.innerHTML='';
    const cycles=availablePayCycles().sort((a,b)=>a.startDate.localeCompare(b.startDate));
    const groups={
      awaiting:cycles.filter(c=>smartStatus(c)==='awaiting'),
      current:cycles.filter(c=>smartStatus(c)==='current'),
      future:cycles.filter(c=>smartStatus(c)==='future'),
      history:cycles.filter(c=>['paid','past'].includes(smartStatus(c))).sort((a,b)=>b.startDate.localeCompare(a.startDate))
    };
    paySection('Awaiting Pay',groups.awaiting,wrap);
    paySection('Current Cycle',groups.current,wrap);
    paySection('Future Cycles',groups.future,wrap);

    if(groups.history.length){
      const byFy={};
      groups.history.forEach(c=>{
        const fy=financialYearFor(parseDate(c.startDate)).label;
        (byFy[fy]??=[]).push(c);
      });
      Object.entries(byFy)
        .sort((a,b)=>b[0].localeCompare(a[0]))
        .forEach(([fy,items])=>paySection(`Pay History · ${fy}`,items,wrap));
    }
    if(!wrap.children.length)wrap.innerHTML='<div class="empty-state">No pay cycles saved yet.</div>';
  }

  function renderPayScreen(){
    const cycles=availablePayCycles();
  ...
  const isWorkingRow=row=>{
    const entered=row?.entered ?? Boolean(row?.code);
    return Boolean(entered&&row?.code&&row.type!=='Off'&&!SHIFT_DATA[row.code]?.leaveType);
  };
  const shiftShareClass=row=>{
    const [h,m]=(row.start||'00:00').split(':').map(Number);
    const start=h*60+m;
    if(start>=19*60&&start<=20*60)return'Night';
    if(start<12*60)return'Day';
    return'Arvo';
  };
  ...
  function rosterShareEntries(range){
    const entries=rosterTimeline(),today=startOfRosterDay(new Date());
    let from=today,to=null;
    if(range==='current'){
      from=parseDate(current.startDate);to=addRosterDays(from,13);
    }else if(range==='4weeks'){
      to=addRosterDays(today,27);
    }else{
      const future=[...entries.values()].filter(e=>e.date>=today&&isWorkingRow(e.row)).sort((a,b)=>a.date-b.date);
      to=future.length?future[future.length-1].date:today;
    }
    return [...entries.values()]
      .filter(e=>e.date>=from&&e.date<=to&&isWorkingRow(e.row))
      .sort((a,b)=>a.date-b.date);
  }
  async function shareRoster(){
    const range=$('#shareRosterRange')?.value||'4weeks';
    const entries=rosterShareEntries(range);
    if(!entries.length){toast('No working days found in that period');return}
    const body=['BEGIN:VCALENDAR','VERSION:2.0','PRODID:-//PTA ShiftMate//Roster//EN','CALSCALE:GREGORIAN'];
    entries.forEach(e=>{
      const next=addRosterDays(e.date,1);
      body.push('BEGIN:VEVENT');
      body.push(`UID:pta-shiftmate-${e.key}@local`);
      body.push(`DTSTART;VALUE=DATE:${icsDate(e.date)}`);
      body.push(`DTEND;VALUE=DATE:${icsDate(next)}`);
      body.push(`SUMMARY:Working - ${shiftShareClass(e.row)}`);
    ...
    try{
      if(navigator.canShare?.({files:[file]})){
        await navigator.share({title:'PTA ShiftMate roster',files:[file]});
      }else{
        const url=URL.createObjectURL(file),a=document.createElement('a');
        a.href=url;a.download=file.name;document.body.appendChild(a);a.click();a.remove();
        setTimeout(()=>URL.revokeObjectURL(url),1000);
        toast('Roster calendar created');
      }
    }catch(err){
      if(err?.name!=='AbortError')toast('Could not share roster');
    }
  }
  ...
  function renderHomeDashboard(){
    const upcoming=upcomingPayCycle();
    if(upcoming){
      const result=resultForCycle(upcoming);
      $('#homeRange').textContent=rangeLabel(upcoming.startDate);
      ...
      $('#homeRange').textContent='No upcoming pay';
      $('#homeNet').textContent=money(0);
      $('#homeCurrentPayday').textContent='Payday —';
      $('#homeRosterStatus').textContent='No upcoming cycle saved';
    }
    renderWeekStrip();
  }
  ...
  function updateRosterCardState(card){
    const entered=card.dataset.entered==='true';
    card.classList.toggle('roster-unentered',!entered);
    card.classList.toggle('roster-entered',entered);
  }
  ...
  function buildRoster(){
    ...
    current.days.forEach((row,i)=>{
      ...
      card.dataset.entered=String(Boolean(row.entered??row.code));
      ...
      card.querySelector('.shift-code').onchange=()=>{card.dataset.entered='true';applyShiftDefaults(card,date,true);updateRosterCardState(card);syncCurrentFromUI();recalculate()};
  ...
  function recalculate(){
    ...
    $$('.day-card').forEach(updateRosterCardState);
  ...
  $('#backToUpcomingPay').onclick=()=>{
    const c=upcomingPayCycle();
    if(c){selectedPayCycleId=c.id;renderPayResult(c)}
  };
