...
(() => {
  ...
  const defaults={baseRate:44.37,wdMult:1,satMult:1.5,sunMult:2,addHoursMult:1.84,weekendOtMult:2,publicHolidayWorkedMult:2.5,hdRate:4.87,maRate:4.87,nightRate:5.77,lease:546.95,gesb:100,postTax:7.40,annualLeaveLoadingRate:12.55,extraTax:0,customPublicHolidays:''};
  ...
  const normaliseSettings=raw=>Object.fromEntries(Object.keys(defaults).map(k=>[k,k==='customPublicHolidays'?String(raw?.[k]??defaults[k]):Number(raw?.[k]??defaults[k])]));
  ...
  function readSettingsFromForm(){
    Object.keys(defaults).forEach(k=>{if($('#'+k)) current.settings[k]=k==='customPublicHolidays'?$('#'+k).value.trim():(Number($('#'+k).value)||0)});
