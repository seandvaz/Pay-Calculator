...
(() => {
  ...
  const peLabel=start=>{const d=periodEndDate(start);return `PE${String(d.getDate()).padStart(2,'0')}${String(d.getMonth()+1).padStart(2,'0')}${String(d.getFullYear()).slice(-2)}`};
  const cycleDateRange=start=>{const a=parseDate(start),b=periodEndDate(start);return `${a.toLocaleDateString('en-AU',{day:'numeric',month:'short'})} – ${b.toLocaleDateString('en-AU',{day:'numeric',month:'short',year:'numeric'})}`};
  ...
  const rosterVisualKind=entry=>{
    if(!entry?.row?.code)return'unknown';
    const row=entry.row;
    if(row.type==='Off'||SHIFT_DATA[row.code]?.leaveType)return'not-working';
    return'working';
  };
  const rosterShiftBand=row=>{
    if(!row?.code||row.type==='Off'||SHIFT_DATA[row.code]?.leaveType)return'';
    const [h,m]=(row.start||'00:00').split(':').map(Number),mins=h*60+m;
    if(mins>=19*60&&mins<=20*60)return'NIGHT';
    if(mins<12*60)return'DAY';
    return'ARVO';
  };
  ...
  function renderCalendar(selectedKey=''){
    ...
    for(let date=new Date(first);date<=last;date=addRosterDays(date,1)){
      ...
      const band=rosterShiftBand(row),display=rosterDisplayCode(entry);
      button.innerHTML=`<span class="calendar-date">${date.getDate()}</span>
        ${key===todayKey?'<span class="today-marker">TODAY</span>':''}
        <strong>${band||display}</strong><small>${band&&display?display:'&nbsp;'}</small>`;
  ...
  function renderHomeCycleJump(){
    ...
    const cycles=availableCycles().sort((a,b)=>b.startDate.localeCompare(a.startDate));
    select.innerHTML=cycles.map(c=>`<option value="${c.id}">${peLabel(c.startDate)} · ${cycleDateRange(c.startDate)}</option>`).join('');
    const match=cycles.find(c=>c.startDate===current.startDate),upcoming=upcomingPayCycle();
    select.value=(match||upcoming||cycles[0])?.id||'';
  ...
  function updateRosterCardState(card){
    const hasRosterEntry=Boolean(card.querySelector('.shift-code')?.value);
    card.dataset.entered=String(hasRosterEntry);
    card.classList.toggle('roster-unentered',!hasRosterEntry);
    card.classList.toggle('roster-entered',hasRosterEntry);
  }
  ...
  function makePayCard(c){
    ...
    card.querySelector('.open-pay-roster').onclick=()=>{loadCycleIntoRoster(c);go('roster');toast(`${peLabel(c.startDate)} opened in roster`)};
  ...
  $('#homeCycleRoster').onclick=()=>{
    const cycle=availableCycles().find(c=>c.id===$('#homeCycleSelect').value);if(!cycle)return;
    loadCycleIntoRoster(cycle);go('roster');toast(`${peLabel(cycle.startDate)} loaded`);
  };
