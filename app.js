  ...
  function renderCalendar(selectedKey=''){
    ...
    for(let offset=0;offset<18;offset++){
      ...
      for(let day=1;day<=daysInMonth;day++){
        ...
        const back=otRec?`<span class="ot-flip-stars ${otRec.stars===2?'ot-best':'ot-good'}">${otRec.stars===2?'★':'☆'}</span><strong>${otRec.stars===2?'Best OT':'Good OT'}</strong><b>+${money(otRec.singleNetGain)}</b><small>net</small>`:'';
        ...
        button.onclick=()=>{
          if(otRec){
            ...
            if(wasFlipped){hideCalendarPaySummary();return}
