  ...
  function buildRoster(){
    ...
    current.days.forEach((row,i)=>{
      ...
      card.querySelector('.shift-code').onchange=()=>{
        const main=card.querySelector('.shift-code').value;
        if(main===OFFLINE_CODE){offlineWrap.hidden=false;card.dataset.entered='true';card.dataset.offline='true';const code=offlineSelect.value||card.dataset.effectiveShiftCode||'';if(code){card.dataset.effectiveShiftCode=code;applyShiftDefaults(card,date,true)}}
