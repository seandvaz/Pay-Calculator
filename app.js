  ...
  function renderCalendar(selectedKey=''){
    ...
    for(let offset=0;offset<18;offset++){
      ...
      for(let day=1;day<=daysInMonth;day++){
        ...
        const front=`${key===todayKey?'<span class="today-marker">TODAY</span>':''}${starText?`<span class="ot-stars" aria-label="${stars===2?'Best OT':'Good OT'}">${starText}</span>`:''}<span class="calendar-date">${day}</span><strong>${label}</strong>`;
        const back=otRec?`<span class="ot-flip-stars">${otRec.stars===2?'★':'☆'}</span><strong>${otRec.stars===2?'Best OT':'Good OT'}</strong><b>+${money(otRec.netGain)}</b><small>net</small>`:'';
        ...
        button.onclick=()=>{
          if(whatIfState.action==='swap'&&whatIfState.swapFrom&&whatIfState.swapFrom!==key){
            const from=parseDate(whatIfState.swapFrom),sim=simulateWhatIf(from,'swap','',date),panel=$('#whatIfPanel');
            if(panel&&sim){panel.hidden=false;panel.querySelector('.what-if-result').innerHTML=whatIfResultMarkup(sim);whatIfState.action='';whatIfState.swapFrom=''}
            return;
          }
          if(otRec){
          ...
          renderWhatIfPanel(date,entry,projection)
        };
