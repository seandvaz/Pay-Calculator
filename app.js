...
(() => {
  ...
  const rosterVisualKind=row=>{
    if(!row?.code||row.type==='Off')return'off';
    if(SHIFT_DATA[row.code]?.leaveType)return'leave';
  ...
  function renderWeekStrip(){
    ...
    for(let i=0;i<7;i++){
      ...
      button.type='button';
      button.className=`week-day ${key===todayKey?'today':''} ${rosterVisualKind(row)}`;
  ...
  function renderCalendar(selectedKey=''){
    ...
    for(let day=1;day<=daysInMonth;day++){
      ...
      button.className=`calendar-day ${rosterVisualKind(row)} ${key===todayKey?'today':''} ${key===selectedKey?'selected':''}`;
