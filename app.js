...
(() => {
  ...
  const defaults={rateSource:'agreement',employmentRole:'STO',toCommencementDate:'',fcoAppointmentDate:'',stoPromotionDate:'',previousRoleBeforeSto:'FCO',classificationOverride:true,classification:'STO3',manualBaseRate:44.37,baseRate:44.37,wdMult:1,satMult:1.5,sunMult:2,addHoursMult:1.84,weekendOtMult:2,publicHolidayWorkedMult:2.5,hdRate:4.87,maRate:4.87,nightRate:5.77,lease:546.95,gesb:100,postTax:7.40,annualLeaveLoadingRate:12.55,extraTax:0,customPublicHolidays:'',homeLine:'ARMADALE',rosterLineNumber:4,rosterLineAnchorDate:'2026-08-09',otTarget:1};
  ...
  const emptyDay=()=>({code:'',type:'Rostered',hd:false,start:'',finish:'',earlyStartHours:0,additionalHours:0,leaveHours:'',annualLeaveHours:0,phBenefit:'lieu',offline:false,workedRosterLine:'',offlineReason:'directed',partner:'',entered:false});
  ...
  function renderFortnightStrip(){
    ...
    for(let week=0;week<2;week++){
      ...
      for(let j=0;j<7;j++){
        const i=week*7+j,date=addRosterDays(start,i),key=localISO(date),state=visualDayState(date,entries),button=document.createElement('button');
        button.type='button';button.dataset.key=key;
        const periodClass=state.period==='Morn'||state.period==='Morning'?'shift-morn':state.period==='Arvo'?'shift-arvo':'shift-off';
        button.className=`week-day ${state.actual?'actual':'projected'} ${periodClass} ${key===todayKey?'today':''}`;
        button.innerHTML=`${key===todayKey?'<span class="today-marker">TODAY</span>':''}<span class="week-dow">${date.toLocaleDateString('en-AU',{weekday:'short'})}</span>
          <span class="week-date">${date.getDate()}</span><strong>${state.label||''}</strong>`;
    ...
    requestAnimationFrame(()=>{
      const today=wrap.querySelector('.today');
      const page=today?.closest('.fortnight-week');
      if(page)wrap.scrollTo({left:Number(page.dataset.week||0)*wrap.clientWidth,behavior:'auto'});
    });
  }
  ...
  function projectedFortnightNetEstimate(start){
    ...
    for(let i=0;i<14;i++){
      ...
      days[i]={...emptyDay(),code,type:'Rostered',entered:true,phBenefit:'lieu'};
      shiftCount++;
    }
  ...
  function overtimeRankingForFortnight(date,entries=rosterTimeline()){
    ...
    const cacheKey=`${localISO(start)}|${actualCycle?.id||'projected'}|${current.settings.homeLine}|${current.settings.classification}|${current.settings.rosterLineNumber}|${requestedTarget}`;
    ...
    for(let i=0;i<14;i++){
      ...
      for(const code of codes){
        ...
        const candidate={dateKey:localISO(d),dayIndex:i,code,singleNetGain:result.net-baselineResult.net,singleGrossGain:result.gross-baselineResult.gross};
    ...
    const comboResults=combinations(dayCandidates,target).map(combo=>{
      ...
      return {combo,result,net:result.net,gross:result.gross};
    }).sort((a,b)=>b.net-a.net||b.gross-a.gross||a.combo.map(x=>x.dayIndex).join(',').localeCompare(b.combo.map(x=>x.dayIndex).join(',')));
    const bestCombo=comboResults[0]||null;
    const secondCombo=comboResults.find(c=>c.net<(bestCombo?.net??0)-0.005)||comboResults.find(c=>c!==bestCombo)||null;
    const annotate=(comboResult,stars)=>{
      if(!comboResult)return;
      comboResult.combo.forEach(candidate=>{
        if(ranking.has(candidate.dateKey))return;
    ...
    annotate(bestCombo,2);
    if(secondCombo)secondCombo.combo.filter(c=>!ranking.has(c.dateKey)).forEach(candidate=>{
      const without=secondCombo.combo.filter(x=>x.dateKey!==candidate.dateKey);
      const withoutNet=PayCalc.calculate(applyOtCandidates(baseline,without)).net;
      ranking.set(candidate.dateKey,{...candidate,stars:1,target,comboNet:secondCombo.net,comboGross:secondCombo.gross,netGain:secondCombo.net-withoutNet,grossGain:candidate.singleGrossGain});
    });
  ...
  function renderCalendar(selectedKey=''){
    ...
    if(status){
      if(profile){status.hidden=true;status.textContent=''}
        ...
        const role=projectionRoleForSettings(),roleLabel=({TRANSIT:'Transit',SENIOR_TRANSIT:'Senior Transit',SUPERVISOR:'Supervisor'})[role]||'selected role';
        status.hidden=false;status.textContent=`Projected roster unavailable for ${current.settings.homeLine||'this home line'} · ${roleLabel}. Actual rosters are unaffected.`;
      }
    }
    const focus=selectedKey?parseDate(selectedKey):calendarCursor;
    const firstMonth=new Date(focus.getFullYear(),focus.getMonth(),1);
    wrap.innerHTML='';
    for(let offset=0;offset<18;offset++){
      ...
      const header=document.createElement('h3');header.className='calendar-scroll-month';header.textContent=monthDate.toLocaleDateString('en-AU',{month:'long',year:'numeric'});section.appendChild(header);
      ...
      for(let day=1;day<=daysInMonth;day++){
        ...
        const candidateProjection=rosterProjectionForDate(date),fortnightStart=candidateProjection?.fortnightStart||null;
        ...
        const stars=otStarsForDate(date,entries),starText=stars?'★'.repeat(stars):'';
        const otRec=stars?otRecommendationForDate(date,entries):null;
        button.className=`calendar-day ${isActual?'actual':'projected'} ${periodClass} ${key===todayKey?'today':''} ${key===selectedKey?'selected':''} ${otRec?'ot-recommended':''}`;
        const front=`${key===todayKey?'<span class="today-marker">TODAY</span>':''}${starText?`<span class="ot-stars" aria-label="${stars===2?'Best OT value':'OT value'}">${starText}</span>`:''}<span class="calendar-date">${day}</span><strong>${label}</strong>`;
        const back=otRec?`<span class="ot-flip-stars">${'★'.repeat(otRec.stars)}</span><strong>${otRec.stars===2?'Best OT':'OT value'}</strong><b>+${money(otRec.netGain)}</b><small>net</small>`:'';
        ...
        button.onclick=()=>{
          if(otRec){
            ...
            hideCalendarPaySummary();$('#calendarDetail').hidden=true;
            if(!wasFlipped){button.classList.add('flipped','selected')}
            return;
          }
          $$('.calendar-day.flipped').forEach(el=>el.classList.remove('flipped','selected'));
          ...
          else{$('#calendarDetail').hidden=true;renderActualPaySummary(entry)}
        };
  ...
  function renderCalendarDetail(key,entry){
    ...
    detail.innerHTML=`<span class="eyebrow">${date.toLocaleDateString('en-AU',{weekday:'long',day:'numeric',month:'long',year:'numeric'})}</span>
      ...
      <div class="calendar-detail-grid">
        ...
        ${row.partner?`<div><span>Partner</span><strong>${row.partner}</strong></div>`:''}
  ...
  // Projected-roster profiles are keyed by home line + role inferred from Current level.
  ...
  function projectionProfileKey(settings=current.settings){return `${settings?.homeLine||''}|${projectionRoleForSettings(settings)}`}
  ...
  function rosterProjectionForDate(date){
    ...
    return {profile,line,day:day+1,mark,working:Boolean(mark),shiftType:mark==='M'?'Morning':mark==='X'?'Arvo':'Off',fortnightStart};
  }
  ...
  const stringSettings=new Set(['rateSource','employmentRole','toCommencementDate','fcoAppointmentDate','stoPromotionDate','previousRoleBeforeSto','classification','customPublicHolidays','homeLine','rosterLineAnchorDate']);
