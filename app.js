...
const perthShiftLabels={PN:'Perth Assist Arvo',PA:'Perth Afternoon',PD:'Perth Assist Day',PM:'Perth Morning',PRD:'Perth Roaming Day',UD:'Underground Day',UN:'Underground Arvo'};
  ...
  const resolveShiftCode=raw=>{
    const value=String(raw||'').trim();
    if(!value)return '';
    if(SHIFT_DATA[value])return value;
    const upper=value.toUpperCase();
    const tango=upper.match(/^TANGO\s+(\d+)$/);
    if(tango&&SHIFT_DATA[tango[1]])return tango[1];
    const delta=upper.match(/^DELTA\s+(\d+)\s+(MORNING|AFTERNOON|NIGHT)$/);
    if(delta){
      const suffix=delta[2]==='MORNING'?'M':delta[2]==='AFTERNOON'?'A':'N';
      const code=delta[1]+suffix;
      if(SHIFT_DATA[code])return code;
    }
    if(upper==='STANDBY'&&SHIFT_DATA.SA)return 'SA';
    return value;
  };
  ...
  const cardShiftCode=card=>{
    const main=card.querySelector('.shift-code')?.value||'';
    if(main===OFFLINE_CODE){
      const stored=card.dataset.effectiveShiftCode||'';
      const selected=card.querySelector('.offline-shift-code')?.value||stored;
      return resolveShiftCode(selected)||resolveShiftCode(stored);
    }
    return resolveShiftCode(main);
  };
  ...
  function applyShiftDefaults(card,date,force=false){
    const code=resolveShiftCode(cardShiftCode(card));
    ...
    if(!data){
      if(!code){
        start.value='';finish.value='';type.value='Off';hd.value='no';label.textContent='Off / no shift';
      }else{
        label.textContent='Shift data unavailable';
      }
      return;
    }
    ...
    const selectedData=SHIFT_DATA[code];
  ...
  function buildRoster(){
    ...
    current.days.forEach((row,i)=>{
      ...
      card.querySelector('.shift-code').onchange=()=>{
        const main=card.querySelector('.shift-code').value;
        const field=card.querySelector('.offline-shift-wrap');
        const os=card.querySelector('.offline-shift-code');
        if(main===OFFLINE_CODE){
          if(field)field.hidden=false;
          if(os)os.innerHTML=allNetworkOpts(os.value,date);
          card.dataset.offline='true';
          card.dataset.entered='true';
          const code=resolveShiftCode(os?.value||card.dataset.effectiveShiftCode||'');
          if(code)card.dataset.effectiveShiftCode=code;
      ...
      card.querySelector('.offline-shift-code').onchange=e=>{
        const code=resolveShiftCode(e.target.value);
        const data=SHIFT_DATA[code];
        if(!data)return;
        ...
        const line=card.querySelector('.worked-line');
        if(line){
          line.value=data.line||current.settings.homeLine;
          card.dataset.workedRosterLine=line.value;
        }
        applyShiftDefaults(card,date,true);
        syncCardShiftDisplay(card);
        updateRosterCardState(card);
      ...
      offlineShiftCode:card.querySelector('.shift-code').value===OFFLINE_CODE?cardShiftCode(card):'',type:card.querySelector('.shift-type').value,start:st,finish:fn};
      ...
      if(row.offlineShiftCode){
        const offlineCode=resolveShiftCode(row.offlineShiftCode);
        const os=card.querySelector('.offline-shift-code');
        if(os)os.value=offlineCode;
        card.querySelector('.offline-shift-wrap').hidden=false;
        card.querySelector('.shift-code').value=OFFLINE_CODE;
        card.dataset.effectiveShiftCode=offlineCode;
        syncCardShiftDisplay(card);
      }
