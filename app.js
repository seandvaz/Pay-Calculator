...
(() => {
  ...
  const defaults={rateSource:'agreement',classification:'STO3',manualBaseRate:44.37,nextIncrementDate:'',autoIncrement:false,baseRate:44.37,wdMult:1,satMult:1.5,sunMult:2,addHoursMult:1.84,weekendOtMult:2,publicHolidayWorkedMult:2.5,hdRate:4.87,maRate:4.87,nightRate:5.77,lease:546.95,gesb:100,postTax:7.40,annualLeaveLoadingRate:12.55,extraTax:0,customPublicHolidays:''};
  ...
  const stringSettings=new Set(['rateSource','classification','nextIncrementDate','customPublicHolidays']);
  const boolSettings=new Set(['autoIncrement']);
  ...
  function updateAgreementSettingsUI(){
    ...
    if($('#agreementRateSummary'))$('#agreementRateSummary').textContent=manual
      ...
      :`${agreement.classificationLabel}: ${money(agreement.weeklyRate)} per week / ${money(agreement.baseRate)} per hour, wage table effective ${agreement.wageEffective}.`;
  ...
  ['rateSource','classification','manualBaseRate','nextIncrementDate','autoIncrement'].forEach(id=>{
