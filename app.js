...
(() => {
  ...
  function renderProjectedPaySummary(date){
    ...
    if(projectionFortnightHasActual(start,entries)){detail.hidden=true;return}
    ...
    detail.innerHTML=`<span class="eyebrow">Projected pay · ${start.toLocaleDateString('en-AU',{day:'numeric',month:'short'})} – ${end.toLocaleDateString('en-AU',{day:'numeric',month:'short'})}</span>
      ...
      <small>Planning estimate based on the projected Morn/Arvo pattern. Your entered roster always takes precedence.</small>`;
  }
  function renderCalendar(selectedKey=''){
    ...
    for(let offset=0;offset<18;offset++){
      ...
      for(let day=1;day<=daysInMonth;day++){
        ...
        const fortnightStart=projectionFortnightStart(date),actualFortnight=projectionFortnightHasActual(fortnightStart,entries);
        const projection=!actualFortnight?seniorProjectionForDate(date):null;
        ...
        button.onclick=()=>{
          ...
          else{$('#calendarProjectionPay').hidden=true;renderCalendarDetail(key,entry)}
        };
        grid.appendChild(button);
      }
      section.appendChild(grid);wrap.appendChild(section);
    }
    $('#calendarDetail').hidden=true;$('#calendarProjectionPay').hidden=true;
