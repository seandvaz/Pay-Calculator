...
const perthShiftLabels={PN:'Perth Assist Arvo',PA:'Perth Afternoon',PD:'Perth Assist Day',PM:'Perth Morning',PRD:'Perth Roaming Day',UD:'Underground Day',UN:'Underground Arvo'};
  ...
  const effectiveShiftCode=card=>{
    ...
    if(main===OFFLINE_CODE)return card.dataset.effectiveShiftCode||card.querySelector('.offline-shift-code')?.value||'';
    return main;
  };
  ...
  function buildRoster(){
    ...
    current.days.forEach((row,i)=>{
      ...
      <div class="day-main">
        <label class="shift-picker"><span class="shift-face-code">${row.code?(/^\d+$/.test(row.code)?`T${row.code}`:row.code):'OFF'}</span><select class="shift-code" aria-label="Select shift">${opts(row.code,initialLine,date,true)}</select></label>
      ...
      if(row.offlineShiftCode){offlineSelect.value=row.offlineShiftCode;offlineWrap.hidden=false;card.dataset.effectiveShiftCode=row.offlineShiftCode;}
      card.querySelector('.shift-code').onchange=()=>{
        const main=card.querySelector('.shift-code').value;
        if(main===OFFLINE_CODE){offlineWrap.hidden=false;card.dataset.entered='true';const code=offlineSelect.value||card.dataset.effectiveShiftCode||'';if(code){card.dataset.effectiveShiftCode=code;applyShiftDefaults(card,date,true)}}
        else{offlineWrap.hidden=true;offlineSelect.value='';delete card.dataset.effectiveShiftCode;applyShiftDefaults(card,date,true)}
      ...
      offlineSelect.onchange=()=>{
        ...
        const line=card.querySelector('.worked-line');if(line){line.value=data.line||current.settings.homeLine;card.dataset.workedRosterLine=line.value}
