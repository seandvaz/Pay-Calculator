...
(() => {
  ...
  const defaults={rateSource:'agreement',employmentRole:'STO',toCommencementDate:'',fcoAppointmentDate:'',stoPromotionDate:'',previousRoleBeforeSto:'FCO',classificationOverride:true,classification:'STO3',manualBaseRate:44.37,baseRate:44.37,wdMult:1,satMult:1.5,sunMult:2,addHoursMult:1.84,weekendOtMult:2,publicHolidayWorkedMult:2.5,hdRate:4.87,maRate:4.87,nightRate:5.77,lease:546.95,gesb:100,postTax:7.40,annualLeaveLoadingRate:12.55,extraTax:0,customPublicHolidays:'',homeLine:'ARMADALE',rosterRole:'SENIOR_TRANSIT'};
  ...
  function renderCalendar(selectedKey=''){
    ...
    if(status){
      if(profile){status.hidden=true;status.textContent=''}
        ...
        const roleLabel=({TRANSIT:'Transit',SENIOR_TRANSIT:'Senior Transit',SUPERVISOR:'Supervisor'})[current.settings.rosterRole]||'selected role';
    ...
    for(let offset=0;offset<18;offset++){
      ...
      for(let day=1;day<=daysInMonth;day++){
        ...
        const actualFortnight=fortnightStart?projectionFortnightHasActual(fortnightStart,entries):false;
        ...
        let kind=rosterVisualKind(entry),label=rosterDisplayCode(entry),projectedClass='';
        if(projection){
          kind='';
          label=projection.shiftType==='Morning'?'Morn':projection.shiftType==='Arvo'?'Arvo':'';
          projectedClass=projection.shiftType==='Morning'?'projected-morn':projection.shiftType==='Arvo'?'projected-arvo':'projected-off';
        }
        button.className=`calendar-day ${kind} ${projectedClass} ${key===todayKey?'today':''} ${key===selectedKey?'selected':''}`;
        button.innerHTML=`${key===todayKey?'<span class="today-marker">TODAY</span>':''}<span class="calendar-date">${day}</span><strong>${label}</strong>`;
        button.onclick=()=>{
  ...
  // Projected-roster profiles are keyed by home line + roster role.
  // A profile is planning-only and never writes to actual roster/pay data.
  const ROSTER_PROJECTION_PROFILES={
    'ARMADALE|SENIOR_TRANSIT':{
      label:'Armadale · Senior Transit',
      // Confirmed anchor: Line 4, Day 12 = Thu 20 Aug 2026; Line 4 Day 1 = Sun 9 Aug 2026.
      anchor:'2026-08-09',
      lineSequence:[4,5,6,1,2,3],
      pattern:{
        1:['X','X','','','','X','X','X','X','X','X','','',''],
        2:['','','','X','X','X','X','X','','','X','X','X',''],
        3:['','M','M','M','M','','','','','','M','M','M','M'],
  ...
  function projectionProfileKey(settings=current.settings){
    return `${settings?.homeLine||''}|${settings?.rosterRole||''}`;
  }
  function currentProjectionProfile(){
    const raw=ROSTER_PROJECTION_PROFILES[projectionProfileKey()];
    if(!raw)return null;
    return {...raw,anchorDate:parseDate(raw.anchor)};
  }
  function rosterProjectionForDate(date){
    const profile=currentProjectionProfile();if(!profile)return null;
    const d=startOfRosterDay(date),days=Math.round((d-profile.anchorDate)/86400000);
    const fortnight=Math.floor(days/14),day=mod(days,14),line=profile.lineSequence[mod(fortnight,profile.lineSequence.length)];
    const mark=profile.pattern[line]?.[day]||'';
    return {profile,line,day:day+1,mark,working:Boolean(mark),shiftType:mark==='M'?'Morning':mark==='X'?'Arvo':'Off',fortnightStart:addRosterDays(profile.anchorDate,Math.floor(days/14)*14)};
  }
  ...
  const stringSettings=new Set(['rateSource','employmentRole','toCommencementDate','fcoAppointmentDate','stoPromotionDate','previousRoleBeforeSto','classification','customPublicHolidays','homeLine','rosterRole']);
  ...
  ['classification','homeLine','rosterRole'].forEach(id=>{
