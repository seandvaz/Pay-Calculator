...
(() => {
  ...
  const emptyDay=()=>({code:'',type:'Rostered',hd:false,start:'',finish:'',earlyStartHours:0,additionalHours:0,leaveHours:'',annualLeaveHours:0,phBenefit:'lieu',offline:false,workedRosterLine:'',offlineReason:'directed',entered:false});
  ...
  function renderWeekStrip(){
    ...
    const start=addRosterDays(sundayOfRosterWeek(new Date()),weekOffset*7);
  ...
  function renderCalendar(selectedKey=''){
    ...
    const first=sundayOfRosterWeek(firstOfMonth),last=addRosterDays(sundayOfRosterWeek(lastOfMonth),6);
  ...
  // Senior projected roster: planning-only layer. Never writes to roster/pay data.
  // Anchor confirmed by user: Line 4, Day 12 = Thu 20 Aug 2026; therefore Line 4 Day 1 = Sun 9 Aug 2026.
  const SENIOR_PROJECTION_ANCHOR=parseDate('2026-08-09');
  const SENIOR_LINE_SEQUENCE=[4,5,6,1,2,3];
  ...
  function seniorProjectionForDate(date){
    const d=startOfRosterDay(date),days=Math.round((d-SENIOR_PROJECTION_ANCHOR)/86400000);
    const fortnight=Math.floor(days/14),day=mod(days,14),line=SENIOR_LINE_SEQUENCE[mod(fortnight,6)];
    const mark=SENIOR_PATTERN[line][day]||'';
    return {line,day:day+1,mark,working:Boolean(mark),shiftType:mark==='M'?'Morning':mark==='X'?'Arvo':'Off'};
  }
  ...
  function projectedDayGross(date,code){
    const settings=projectedSettingsForDate(date);
    const makeDays=()=>Array.from({length:14},()=>({...emptyDay(),type:'Off'}));
    const baselineDays=makeDays(),candidateDays=makeDays();
    candidateDays[0]={...emptyDay(),code,type:'Rostered',entered:true,phBenefit:'lieu'};
    const startDate=localISO(date);
    const baseline=PayCalc.calculate({startDate,settings,days:baselineDays}).gross;
    const candidate=PayCalc.calculate({startDate,settings,days:candidateDays}).gross;
    return Math.max(0,candidate-baseline);
  }
  function projectedFortnightEstimate(start){
    const p0=seniorProjectionForDate(start),values=[];let shiftCount=0;
    for(let i=0;i<14;i++){
      const date=addRosterDays(start,i),p=seniorProjectionForDate(date);
      if(!p.working)continue;shiftCount++;
      const codes=projectedCandidateCodes(date,p.shiftType);
      const pays=codes.map(code=>projectedDayGross(date,code)).filter(Number.isFinite).sort((a,b)=>a-b);
      if(!pays.length)continue;
      values.push({min:pays[0],mid:pays[Math.floor((pays.length-1)/2)],max:pays[pays.length-1]});
    }
    const sum=k=>values.reduce((t,v)=>t+v[k],0);
    return {line:p0.line,shiftCount,min:sum('min'),mid:sum('mid'),max:sum('max'),complete:values.length===shiftCount};
  }
  function projectionFortnightStart(date){
    const days=Math.floor((startOfRosterDay(date)-SENIOR_PROJECTION_ANCHOR)/86400000);
    return addRosterDays(SENIOR_PROJECTION_ANCHOR,Math.floor(days/14)*14);
  }
  function renderProjectedPayCards(year,month){
    const wrap=$('#projectedPayCards');if(!wrap)return;
    const monthStart=new Date(year,month,1),monthEnd=new Date(year,month+1,0);
    let start=projectionFortnightStart(monthStart);const cards=[];
    while(start<=monthEnd){
      const end=addRosterDays(start,13),est=projectedFortnightEstimate(start);
      cards.push(`<div class="projected-pay-card"><div><span class="eyebrow">Line ${est.line} · ${start.toLocaleDateString('en-AU',{day:'numeric',month:'short'})} – ${end.toLocaleDateString('en-AU',{day:'numeric',month:'short'})}</span><strong>${money(est.mid)}</strong><small>Projected gross · ${est.shiftCount} rostered days</small></div><div class="projected-pay-range"><span>Possible range</span><b>${money(est.min)} – ${money(est.max)}</b></div></div>`);
      start=addRosterDays(start,14);
    }
    wrap.innerHTML=cards.join('');
  }

  function renderProjectedRoster(){
    const grid=$('#projectedGrid');if(!grid)return;
    const entries=rosterTimeline(),year=calendarCursor.getFullYear(),month=calendarCursor.getMonth();
    $('#projectedMonthLabel').textContent=calendarCursor.toLocaleDateString('en-AU',{month:'long',year:'numeric'});
    renderProjectedPayCards(year,month);
    grid.innerHTML='';
    const firstOfMonth=new Date(year,month,1),lastOfMonth=new Date(year,month+1,0);
    const first=sundayOfRosterWeek(firstOfMonth),last=addRosterDays(sundayOfRosterWeek(lastOfMonth),6),todayKey=localISO(new Date());
    for(let date=new Date(first);date<=last;date=addRosterDays(date,1)){
      const key=localISO(date),actual=entries.get(key),p=seniorProjectionForDate(date),outside=date.getMonth()!==month;
      const hasActual=Boolean(actual?.row?.entered ?? actual?.row?.code);
      const b=document.createElement('button');b.type='button';
      b.className=`calendar-day projected-day ${p.working?'projected-working':'projected-off'} ${hasActual?'has-actual':''} ${outside?'outside-month':''} ${key===todayKey?'today':''}`;
      b.innerHTML=`${key===todayKey?'<span class="today-marker">TODAY</span>':''}<span class="calendar-date">${date.getDate()}</span><strong>${hasActual?'ACTUAL':(p.shiftType==='Morning'?'Morn':p.shiftType)}</strong><small>L${p.line} · D${p.day}</small>`;
      b.onclick=()=>{const detail=$('#projectedDetail');detail.hidden=false;detail.innerHTML=`<span class="eyebrow">${date.toLocaleDateString('en-AU',{weekday:'long',day:'numeric',month:'long',year:'numeric'})}</span><h2>${hasActual?'Actual roster entered':`Projected: ${p.shiftType}`}</h2><p>${hasActual?'Projection suppressed for this date. Your entered roster remains authoritative.':`Senior Line ${p.line}, Day ${p.day}. Planning only — no shift/pay data is created.`}</p>`};
      grid.appendChild(b);
    }
  }
  ...
  function go(id){
    ...
    if(id==='projected') renderProjectedRoster();
  ...
  function buildRoster(){
    ...
    current.days.forEach((row,i)=>{
      ...
      card.querySelectorAll('.shift-type,.higher-duties,.ph-benefit,.offline-reason').forEach(el=>el.onchange=()=>{card.dataset.entered='true';updateRosterCardState(card);syncCurrentFromUI();recalculate()});
      card.querySelectorAll('.leave-hours,.annual-leave-hours').forEach(el=>el.oninput=()=>{card.dataset.entered='true';updateRosterCardState(card);syncCurrentFromUI();recalculate()});
      const manualTime=()=>{
  ...
  $('#projectedPrev').onclick=()=>{calendarCursor=new Date(calendarCursor.getFullYear(),calendarCursor.getMonth()-1,1);renderProjectedRoster()};
  $('#projectedNext').onclick=()=>{calendarCursor=new Date(calendarCursor.getFullYear(),calendarCursor.getMonth()+1,1);renderProjectedRoster()};
  $('#projectedToday').onclick=()=>{const t=new Date();calendarCursor=new Date(t.getFullYear(),t.getMonth(),1);renderProjectedRoster()};
