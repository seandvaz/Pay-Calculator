...
const perthShiftLabels={PN:'Perth Assist Arvo',PA:'Perth Afternoon',PD:'Perth Assist Day',PM:'Perth Morning',PRD:'Perth Roaming Day',UD:'Underground Day',UN:'Underground Arvo'};
  ...
  function syncCardShiftDisplay(card){
    const code=cardShiftCode(card);
    const face=card.querySelector('.shift-face-code');
    if(face)face.textContent=code?(code.startsWith('Tango ')||code.startsWith('Delta ')?code:(/^\d+$/.test(code)?`T${code}`:code)):'OFF';
    return code;
  }
  ...
  function updateRosterCardState(card){
    const mainCode=card.querySelector('.shift-code')?.value||'';
    const code=cardShiftCode(card);
    const rowType=card.querySelector('.shift-type')?.value||'';
    const isOvertime=rowType==='Picked-up OT'||rowType==='Overtime';
    // Selecting Off-Line Shift is itself an entered/interactive roster state,
    // even before the underlying network shift code has been chosen.  Keep the
    // OT and Details controls available so the user can open Details and choose
    // the underlying code.
    const hasRosterEntry=Boolean(code)||mainCode===OFFLINE_CODE;
  ...
  function buildRoster(){
    ...
    current.days.forEach((row,i)=>{
      ...
      card.querySelector('.shift-code').onchange=()=>{
        ...
        else{if(field)field.hidden=true;if(os)os.value='';const line=card.querySelector('.worked-line');if(line){line.value=current.settings.homeLine;card.dataset.workedRosterLine=line.value}refreshShiftOptions(card,date,main);applyShiftDefaults(card,date,true);const face=card.querySelector('.shift-face-code');if(face)face.textContent=main?(/^[0-9]+$/.test(main)?`T${main}`:main):'OFF';}
        updateRosterCardState(card);syncCurrentFromUI();recalculate()
      };
      card.querySelector('.offline-shift-code').onchange=e=>{const code=e.target.value,data=SHIFT_DATA[code];if(!data)return;const line=card.querySelector('.worked-line');if(line){line.value=data.line||current.settings.homeLine;card.dataset.workedRosterLine=line.value}card.dataset.offline='true';applyShiftDefaults(card,date,true);syncCardShiftDisplay(card);updateRosterCardState(card);syncCurrentFromUI();recalculate();syncCardShiftDisplay(card)};
      ...
      const manualTime=()=>{
        const data=SHIFT_DATA[cardShiftCode(card)];
        const st=card.querySelector('.start-time').value||'--:--',fn=card.querySelector('.finish-time').value||'--:--';
        const tempRow={code:cardShiftCode(card),
      offlineShiftCode:card.querySelector('.shift-code').value===OFFLINE_CODE?(card.querySelector('.offline-shift-code')?.value||''):'',type:card.querySelector('.shift-type').value,start:st,finish:fn};
  ...
  function syncCurrentFromUI(){
    ...
    current.days=$$('.day-card').map(card=>({
      // For Off-Line Shift, the selected network code is the actual shift code
      // used by the pay engine and displayed on the card. OFFLINE is only the
      // UI selector state that opens the secondary network-code field.
      ...
      offline:(card.querySelector('.worked-line').value||current.settings.homeLine)!==current.settings.homeLine || card.querySelector('.shift-code').value===OFFLINE_CODE,
  ...
  function recalculate(){
    $$('.day-card').forEach(syncCardShiftDisplay);
    syncCurrentFromUI();
    resolveAgreementSettings();
