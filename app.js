...
(() => {
  ...
  function rosterVariation(row,date){
    const data=SHIFT_DATA[row?.code];
    if(!data||data.leaveType||row?.type==='Picked-up OT'||!row?.start||!row?.finish)return {early:0,extension:0};
  ...
  function recalculate(){
    ...
    latestResult.dayTotals.forEach((row,i)=>{
      ...
      if(row.start&&row.finish&&SHIFT_DATA[current.days[i].code]){
        ...
        const variation=rosterVariation(row,date);const earlyText=variation.early?` • ${variation.early.toFixed(2)}h early OT`:'';const addText=variation.extension?` • ${variation.extension.toFixed(2)}h extension OT`:'';
        card.querySelector('.shift-time').textContent=d.leaveType
          ? `${d.name} • ${row.hours.toFixed(1)} paid hours${earlyText}${addText}`
