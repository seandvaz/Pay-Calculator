...
(() => {
  ...
  function renderFortnightStrip(){
    ...
    wrap.innerHTML='';
    for(let i=0;i<14;i++){
      const date=addRosterDays(start,i),key=localISO(date),state=visualDayState(date,entries),button=document.createElement('button');
    ...
    requestAnimationFrame(()=>{
      ...
      if(today)today.scrollIntoView({inline:'center',block:'nearest'});
    });
  }
  ...
  function otValueMultiplier(date){
    if(publicHolidayName(date))return Number(current.settings.publicHolidayWorkedMult)||2.5;
    return (date.getDay()===0||date.getDay()===6)?(Number(current.settings.weekendOtMult)||2):(Number(current.settings.addHoursMult)||1.84);
  }
  function otStarsForDate(date,entries=rosterTimeline()){
    const state=visualDayState(date,entries);if(state.working)return 0;
    const start=state.projection?.fortnightStart||projectionFortnightStartForProfile(date)||(()=>{const d=sundayOfRosterWeek(date);return d})();
    const premium=[];
    for(let i=0;i<14;i++){
      const d=addRosterDays(start,i),st=visualDayState(d,entries);if(st.working)continue;
      const mult=otValueMultiplier(d);if(mult>=2)premium.push(mult);
    }
    if(!premium.length)return 0;
    const distinct=[...new Set(premium)].sort((a,b)=>b-a),mine=otValueMultiplier(date);
    if(mine<2)return 0;
    return mine===distinct[0]?2:1;
  }
