...
(() => {
  ...
  const defaults={rateSource:'agreement',employmentRole:'STO',toCommencementDate:'',fcoAppointmentDate:'',stoPromotionDate:'',previousRoleBeforeSto:'FCO',classificationOverride:true,classification:'STO3',manualBaseRate:44.37,baseRate:44.37,wdMult:1,satMult:1.5,sunMult:2,addHoursMult:1.84,weekendOtMult:2,publicHolidayWorkedMult:2.5,hdRate:4.87,maRate:4.87,nightRate:5.77,lease:546.95,gesb:100,postTax:7.40,annualLeaveLoadingRate:12.55,extraTax:0,customPublicHolidays:'',homeLine:'ARMADALE',rosterType:'SENIOR_TRANSIT',rosterLineNumber:4,rosterLineAnchorDate:'2026-08-09',otTarget:1};
  ...
  function renderCalendar(selectedKey=''){
    ...
    if(status){
      if(profile){status.hidden=true;status.textContent=''}
        ...
        const role=rosterTypeForSettings(),roleLabel=({TRANSIT:'Regular Transit',SENIOR_TRANSIT:'Senior Transit',SUPERVISOR:'Supervisor'})[role]||'selected roster type';
  ...
  // Projected-roster profiles are keyed by home line + roster type.
  // Roster type/line is intentionally independent of employment/pay classification.
  ...
  function rosterTypeForSettings(settings=current.settings){
    const explicit=String(settings?.rosterType||'').toUpperCase();
    if(explicit==='SENIOR_TRANSIT'||explicit==='TRANSIT'||explicit==='SUPERVISOR')return explicit;
  ...
  function renderHomeSettingsSummary(){
    ...
    const rosterTypeLabel=rosterTypeForSettings()==='SENIOR_TRANSIT'?'Senior':'Regular';
    wrap.innerHTML=`<span class="setting-chip"><b>${current.settings.classification||'—'}</b></span>
      ...
      <span class="setting-chip"><small>Roster</small><b>${rosterTypeLabel}</b></span>
  ...
  const stringSettings=new Set(['rateSource','employmentRole','toCommencementDate','fcoAppointmentDate','stoPromotionDate','previousRoleBeforeSto','classification','customPublicHolidays','homeLine','rosterType','rosterLineAnchorDate']);
  ...
  if(current.settings?.rosterType==null){
    current.settings.rosterType=projectionRoleForSettings(current.settings)==='SENIOR_TRANSIT'?'SENIOR_TRANSIT':'TRANSIT';
  }
  ...
  ['classification','homeLine','rosterType','otTarget'].forEach(id=>{
