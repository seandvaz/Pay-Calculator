...
(() => {
  ...
  function renderCalendarDetail(key,entry){
    ...
    detail.innerHTML=`<span class="eyebrow">${date.toLocaleDateString('en-AU',{weekday:'long',day:'numeric',month:'long',year:'numeric'})}</span>
      ...
      <div class="calendar-detail-grid">
        ...
        ${Number(row.earlyStartHours)>0?`<div><span>Early start OT</span><strong>${Number(row.earlyStartHours).toFixed(2)} hrs</strong></div>`:''}
        ${Number(row.additionalHours)>0?`<div><span>Shift extension OT</span><strong>${Number(row.additionalHours).toFixed(2)} hrs</strong></div>`:''}
    ...
    current.days.forEach((row,i)=>{
      ...
      <div class="day-details">
        <div class="form-grid two">
          ...
          <label>Early start OT (hrs)<input class="early-start-hours" type="number" min="0" step="0.25" value="${Number(row.earlyStartHours)||0}"><small class="field-note">Overtime worked before the rostered start time.</small></label>
          <label>Shift extension OT (hrs)<input class="additional-hours" type="number" min="0" step="0.25" value="${Number(row.additionalHours)||0}"><small class="field-note">Overtime worked after the rostered finish time.</small></label>
      ...
      card.querySelectorAll('.early-start-hours,.additional-hours,.leave-hours,.annual-leave-hours').forEach(el=>el.oninput=()=>{card.dataset.entered='true';updateRosterCardState(card);syncCurrentFromUI();recalculate()});
      const manualTime=()=>{
  ...
  function syncCurrentFromUI(){
    ...
    current.days=$$('.day-card').map(card=>({
      ...
      earlyStartHours:Number(card.querySelector('.early-start-hours')?.value)||0,
  ...
  function recalculate(){
    ...
    latestResult.dayTotals.forEach((row,i)=>{
      ...
      if(row.start&&row.finish&&SHIFT_DATA[current.days[i].code]){
        ...
        const earlyText=row.earlyStartHours?` • ${Number(row.earlyStartHours).toFixed(2)}h early OT`:'';const addText=row.additionalHours?` • ${Number(row.additionalHours).toFixed(2)}h extension OT`:'';
        card.querySelector('.shift-time').textContent=d.leaveType
          ? `${d.name} • ${(row.hours-(Number(row.earlyStartHours)||0)-(Number(row.additionalHours)||0)).toFixed(1)} paid hours${earlyText}${addText}`
