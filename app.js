...
(() => {
  ...
  const defaults={rateSource:'agreement',employmentRole:'STO',toCommencementDate:'',fcoAppointmentDate:'',stoPromotionDate:'',previousRoleBeforeSto:'FCO',classificationOverride:true,classification:'STO3',manualBaseRate:44.37,baseRate:44.37,wdMult:1,satMult:1.5,sunMult:2,addHoursMult:1.84,weekendOtMult:2,publicHolidayWorkedMult:2.5,hdRate:4.87,maRate:4.87,nightRate:5.77,lease:546.95,gesb:100,postTax:7.40,annualLeaveLoadingRate:12.55,extraTax:0,customPublicHolidays:'',homeLine:'ARMADALE',rosterLineNumber:4,rosterLineAnchorDate:'2026-08-09'};
  ...
  function overtimeRankingForFortnight(date,entries=rosterTimeline()){
    ...
    const cacheKey=`${localISO(start)}|${actualCycle?.id||'projected'}|${current.settings.homeLine}|${current.settings.classification}|${current.settings.rosterLineNumber}`;
    ...
    const baselineResult=PayCalc.calculate(baseline),options=[];
    for(let i=0;i<14;i++){
      ...
      for(const code of codes){
        ...
        trial.days[i]={...emptyDay(),code,type:'Picked-up OT',entered:true,phBenefit:'lieu'};
        const result=PayCalc.calculate(trial);
        const candidate={dateKey:localISO(d),code,net:result.net,gross:result.gross,netGain:result.net-baselineResult.net,grossGain:result.gross-baselineResult.gross};
        if(!best||candidate.net>best.net+0.005||(Math.abs(candidate.net-best.net)<0.005&&candidate.gross>best.gross))best=candidate;
      }
      if(best)options.push(best);
    }
    options.sort((a,b)=>b.net-a.net||b.gross-a.gross);
    const ranking=new Map();
    if(options.length){
      const bestNet=options[0].net;
      const secondNet=options.find(x=>x.net<bestNet-0.005)?.net;
      options.forEach(x=>{
        if(Math.abs(x.net-bestNet)<0.005)ranking.set(x.dateKey,{stars:2,...x});
        else if(secondNet!=null&&Math.abs(x.net-secondNet)<0.005)ranking.set(x.dateKey,{stars:1,...x});
      });
    }
    otRecommendationCache.set(cacheKey,ranking);return ranking;
  }
  function otStarsForDate(date,entries=rosterTimeline()){
    const state=visualDayState(date,entries);if(state.working)return 0;
    return overtimeRankingForFortnight(date,entries).get(localISO(date))?.stars||0;
  }
  ...
  function renderCalendar(selectedKey=''){
    ...
    const profile=currentProjectionProfile(),status=$('#calendarProjectionStatus');
    if(status){
    ...
    for(let offset=0;offset<18;offset++){
      ...
      for(let day=1;day<=daysInMonth;day++){
        ...
        button.className=`calendar-day ${isActual?'actual':'projected'} ${periodClass} ${key===todayKey?'today':''} ${key===selectedKey?'selected':''}`;
        button.innerHTML=`${key===todayKey?'<span class="today-marker">TODAY</span>':''}${starText?`<span class="ot-stars" aria-label="${stars===2?'Best OT value':'OT value'}">${starText}</span>`:''}<span class="calendar-date">${day}</span><strong>${label}</strong>`;
        button.onclick=()=>{
