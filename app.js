...
(() => {
  ...
  const defaults={rateSource:'agreement',employmentRole:'STO',toCommencementDate:'',fcoAppointmentDate:'',stoPromotionDate:'',previousRoleBeforeSto:'FCO',classificationOverride:true,classification:'STO3',manualBaseRate:44.37,baseRate:44.37,wdMult:1,satMult:1.5,sunMult:2,addHoursMult:1.84,weekendOtMult:2,publicHolidayWorkedMult:2.5,hdRate:4.87,maRate:4.87,nightRate:5.77,lease:546.95,gesb:100,postTax:7.40,annualLeaveLoadingRate:12.55,extraTax:0,customPublicHolidays:''};
  ...
  const emptyDay=()=>({code:'',type:'Rostered',hd:false,start:'',finish:'',additionalHours:0,phBenefit:'cash'});
  ...
  const stringSettings=new Set(['rateSource','employmentRole','toCommencementDate','fcoAppointmentDate','stoPromotionDate','previousRoleBeforeSto','classification','customPublicHolidays']);
  ...
  function buildRoster(){
    ...
    current.days.forEach((row,i)=>{
      ...
      card.querySelectorAll('.shift-type,.higher-duties,.ph-benefit').forEach(el=>el.onchange=()=>{syncCurrentFromUI();recalculate()});card.querySelector('.additional-hours').oninput=()=>{syncCurrentFromUI();recalculate()};
      const manualTime=()=>{
  ...
  function updateAgreementSettingsUI(){
    const agreement=resolveAgreementSettings();
    const manual=current.settings.rateSource==='manual';
    const role=current.settings.employmentRole||'STO';
    const override=Boolean(current.settings.classificationOverride);
    if($('#manualBaseRate'))$('#manualBaseRate').disabled=!manual;
    $$('.manual-rate-field').forEach(el=>el.hidden=!manual);
    $$('.classification-override-field').forEach(el=>el.hidden=!override);
    $$('.to-date').forEach(el=>el.hidden=role==='TRAINEE');
    $$('.fco-date').forEach(el=>el.hidden=!(role==='FCO'||(role==='STO'&&current.settings.previousRoleBeforeSto==='FCO')));
    $$('.sto-date').forEach(el=>el.hidden=role!=='STO');
    if($('#agreementRateSummary'))$('#agreementRateSummary').textContent=manual
      ?`Manual rate active: ${money(agreement.baseRate)} per hour.`
      :`${agreement.classificationLabel}: ${money(agreement.weeklyRate)} per week / ${money(agreement.baseRate)} per hour • ${agreement.classificationSource} • wage table effective ${agreement.wageEffective}.`;
  ...
  ['rateSource','employmentRole','toCommencementDate','fcoAppointmentDate','stoPromotionDate','previousRoleBeforeSto','classificationOverride','classification','manualBaseRate'].forEach(id=>{
