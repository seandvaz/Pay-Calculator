...
const perthShiftLabels={PN:'Perth Assist Arvo',PA:'Perth Afternoon',PD:'Perth Assist Day',PM:'Perth Morning',PRD:'Perth Roaming Day',UD:'Underground Day',UN:'Underground Arvo'};
  ...
  function buildRoster(){
    ...
    current.days.forEach((row,i)=>{
      ...
      offlineSelect.innerHTML=['',...Object.keys(SHIFT_DATA).sort((a,b)=>a.localeCompare(b,undefined,{numeric:true}))].map(code=>`<option value="${code}">${shiftOptionLabel(code)}</option>`).join('');
      if(row.offlineShiftCode){offlineSelect.value=row.offlineShiftCode;offlineWrap.hidden=false;card.dataset.effectiveShiftCode=row.offlineShiftCode;card.dataset.offline='true';}
      card.querySelector('.shift-code').onchange=()=>{
        ...
        if(main===OFFLINE_CODE){
          ...
          const code=offlineSelect.value||card.dataset.effectiveShiftCode||'';
          if(code){card.dataset.effectiveShiftCode=code;applyShiftDefaults(card,date,true)}
          // Off-Line is a two-part selection: choosing it from the card must
          // immediately open Details so the user can choose the actual shift.
          // The Details sheet is portalled out of the card, so open it only
          // after the offline field has been made visible.
          openShiftDetailsPortal(card);
          requestAnimationFrame(()=>offlineSelect.focus());
        }
        else{offlineWrap.hidden=true;offlineSelect.value='';delete card.dataset.effectiveShiftCode;card.dataset.offline='false';applyShiftDefaults(card,date,true)}
        const face=card.querySelector('.shift-face-code');if(face){const code=effectiveShiftCode(card);face.textContent=code?(/^\d+$/.test(code)?`T${code}`:code):'OFF'};
        updateRosterCardState(card);syncCurrentFromUI();recalculate()
      };
      offlineSelect.onchange=()=>{
        const code=offlineSelect.value,data=SHIFT_DATA[code];if(!data)return;
        card.dataset.effectiveShiftCode=code;card.dataset.entered='true';card.dataset.offline='true';
        applyShiftDefaults(card,date,true);
        const face=card.querySelector('.shift-face-code');if(face)face.textContent=/^\d+$/.test(code)?`T${code}`:code;
        updateRosterCardState(card);syncCurrentFromUI();recalculate();
      };
      ...
      const manualTime=()=>{
        ...
        const tempRow={code:effectiveShiftCode(card),type:card.querySelector('.shift-type').value,start:st,finish:fn};
