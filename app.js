...
const perthShiftLabels={PN:'Perth Assist Arvo',PA:'Perth Afternoon',PD:'Perth Assist Day',PM:'Perth Morning',PRD:'Perth Roaming Day',UD:'Underground Day',UN:'Underground Arvo'};
  ...
  const allNetworkOpts=(selected='',date=null)=>{
    const group=date?PayCalc.dayGroup(date.getDay()):null;
    const codes=Object.keys(SHIFT_DATA).filter(code=>{const data=SHIFT_DATA[code];if(!data||data.line==='LEAVE')return false;if(!group)return true;const t=data.times?.[group]||['',''];return Boolean(t[0]&&t[1])}).sort((a,b)=>a.localeCompare(b,undefined,{numeric:true}));
    return ['',...codes].map(code=>`<option value="${code}" ${code===selected?'selected':''}>${code===OFFLINE_CODE?'Off-Line Shift':shiftOptionLabel(code)}</option>`).join('');
  };
  const cardShiftCode=card=>{const main=card.querySelector('.shift-code')?.value||'';return main===OFFLINE_CODE?(card.querySelector('.offline-shift-code')?.value||''):main};
  const opts=(selected,line,date=null)=>{
    ...
    const codes=['',...normal,OFFLINE_CODE,...leaveOrder.filter(code=>SHIFT_DATA[code])];
  ...
  current.days=current.days.map(row=>{
    ...
    const data=SHIFT_DATA[next.offlineShiftCode||next.code];
  ...
  function refreshShiftOptions(card,date,selected=''){
    const line=card.querySelector('.worked-line').value||current.settings.homeLine,select=card.querySelector('.shift-code');
    const field=card.querySelector('.offline-shift-wrap'),os=card.querySelector('.offline-shift-code');
    if(select.value===OFFLINE_CODE){select.innerHTML=opts(OFFLINE_CODE,line,date);select.value=OFFLINE_CODE;if(field)field.hidden=false;if(os)os.innerHTML=allNetworkOpts(os.value,date)}
    else{const validSelected=selected&&SHIFT_DATA[selected]&&(SHIFT_DATA[selected].line==='LEAVE'||(selected==='SA'&&(()=>{const t=SHIFT_DATA[selected].times?.[PayCalc.dayGroup(date.getDay())]||['',''];return Boolean(t[0]&&t[1])})())||(SHIFT_DATA[selected].line===line&&(()=>{const t=SHIFT_DATA[selected].times?.[PayCalc.dayGroup(date.getDay())]||['',''];return Boolean(t[0]&&t[1])})()));select.innerHTML=opts(validSelected?selected:'',line,date);select.value=validSelected?selected:'';if(field)field.hidden=true}
    const offline=line!==current.settings.homeLine||select.value===OFFLINE_CODE;card.dataset.offline=String(offline);const reason=card.querySelector('.offline-reason-wrap');if(reason)reason.hidden=!offline;
  }
  ...
  function buildRoster(){
    ...
    current.days.forEach((row,i)=>{
      ...
      const initialCode=row.offlineShiftCode||row.code||'';
      const initialLine=rowEntered
        ? (row.workedRosterLine||SHIFT_DATA[initialCode]?.line||current.settings.homeLine||'')
      ...
      <div class="day-main">
        <label class="shift-picker"><span class="shift-face-code">${row.code?(/^\d+$/.test(row.code)?`T${row.code}`:row.code):'OFF'}</span><select class="shift-code" aria-label="Select shift">${opts(row.offlineShiftCode?OFFLINE_CODE:row.code,initialLine,date)}</select></label>
      ...
      <div class="day-details">
        ...
        <div class="form-grid two">
          ...
          <label class="offline-shift-wrap" ${row.offlineShiftCode?'':'hidden'}>Off-Line Shift<select class="offline-shift-code">${allNetworkOpts(row.offlineShiftCode||'',date)}</select></label>
      ...
      card.querySelector('.shift-code').onchange=()=>{
        const main=card.querySelector('.shift-code').value,field=card.querySelector('.offline-shift-wrap'),os=card.querySelector('.offline-shift-code');
        if(main===OFFLINE_CODE){if(field)field.hidden=false;if(os)os.innerHTML=allNetworkOpts(os.value,date);card.dataset.offline='true';const face=card.querySelector('.shift-face-code');if(face)face.textContent=os?.value||'OFF';}
        else{if(field)field.hidden=true;if(os)os.value='';const line=card.querySelector('.worked-line');if(line){line.value=current.settings.homeLine;card.dataset.workedRosterLine=line.value}refreshShiftOptions(card,date,main);applyShiftDefaults(card,date,true);const face=card.querySelector('.shift-face-code');if(face)face.textContent=main?(/^\d+$/.test(main)?`T${main}`:main):'OFF';}
        updateRosterCardState(card);syncCurrentFromUI();recalculate()
      };
      card.querySelector('.offline-shift-code').onchange=e=>{const code=e.target.value,data=SHIFT_DATA[code];if(!data)return;const line=card.querySelector('.worked-line');if(line){line.value=data.line||current.settings.homeLine;card.dataset.workedRosterLine=line.value}const face=card.querySelector('.shift-face-code');if(face)face.textContent=code;card.dataset.offline='true';applyShiftDefaults(card,date,true);updateRosterCardState(card);syncCurrentFromUI();recalculate()};
      ...
      refreshShiftOptions(card,date,row.offlineShiftCode||row.code||'');
      if(row.offlineShiftCode){const os=card.querySelector('.offline-shift-code');if(os)os.value=row.offlineShiftCode;card.querySelector('.offline-shift-wrap').hidden=false;card.querySelector('.shift-code').value=OFFLINE_CODE;card.querySelector('.shift-face-code').textContent=row.offlineShiftCode;}
  ...
  function syncCurrentFromUI(){
    ...
    current.days=$$('.day-card').map(card=>({
      code:cardShiftCode(card),
      offlineShiftCode:card.querySelector('.shift-code').value===OFFLINE_CODE?(card.querySelector('.offline-shift-code')?.value||''):'',
  ...
  async function openRosterScanner(mode='current'){
    ...
    try{
      ...
      rosterScanStream=await navigator.mediaDevices.getUserMedia({video:{facingMode:{ideal:'environment'},width:{ideal:1920},height:{ideal:1080}},audio:false});
      const video=$('#rosterScanVideo');video.srcObject=rosterScanStream;await video.play();status.textContent='Fit the full fortnight roster inside the frame, then tap Scan Roster.';
  ...
  function parseOcrDateToken(text,yearHint){
    const t=String(text||'').trim();let m=t.match(/\b(\d{1,2})[\/-](\d{1,2})(?:[\/-](\d{2,4}))?\b/);if(!m)return null;
    let y=m[3]?Number(m[3]):Number(yearHint);if(y<100)y+=2000;const d=Number(m[1]),mo=Number(m[2]);if(!y||d<1||d>31||mo<1||mo>12)return null;
    const date=new Date(y,mo-1,d);return Number.isNaN(date.getTime())?null:date;
  }
  function detectPeriodEnd(text,words=[]){
    const raw=String(text||'');let m=raw.match(/(?:\bPE\b|PERIOD\s*END)\s*[:\-]?\s*(\d{1,2})[\/-]?(\d{1,2})[\/-]?(\d{2,4})/i);
    if(m){let y=Number(m[3]);if(y<100)y+=2000;const d=new Date(y,Number(m[2])-1,Number(m[1]));if(!Number.isNaN(d.getTime()))return d}
    m=raw.match(/\bPE\s*(\d{2})(\d{2})(\d{2,4})\b/i);if(m){let y=Number(m[3]);if(y<100)y+=2000;return new Date(y,Number(m[2])-1,Number(m[1]))}
    const yearMatch=raw.match(/\b(20\d{2})\b/);const year=yearMatch?Number(yearMatch[1]):new Date().getFullYear();
    const dates=words.map(w=>parseOcrDateToken(w.text,year)).filter(Boolean).sort((x,y)=>x-y);
    if(dates.length>=10)return dates[dates.length-1];
    return null;
  }
  function scanAssignmentsFromWords(words,periodEnd){
    ...
    const service=String(current.settings.serviceNumber||'').trim();
    const cleanWords=words.filter(w=>Number(w.confidence??w.conf??0)>25).map(w=>({...w,token:normaliseOcrToken(w.text),box:w.bbox||{x0:0,y0:0,x1:0,y1:0}}));
    const nameParts=employee.split(/\s+/).filter(x=>x.length>2);let targetY=null;
    const identityHits=cleanWords.filter(w=>(service&&w.token===service)||nameParts.some(n=>w.token===normaliseOcrToken(n)));
    if(identityHits.length)targetY=identityHits.reduce((s,w)=>s+(w.box.y0+w.box.y1)/2,0)/identityHits.length;
    const year=periodEnd.getFullYear();
    const dateWords=[];cleanWords.forEach(w=>{const d=parseOcrDateToken(w.text,year);if(d&&d>=addRosterDays(start,-1)&&d<=addRosterDays(periodEnd,1))dateWords.push({w,d})});
    // Grid roster: use date header x positions and the user's row.
    if(targetY!==null&&dateWords.length>=7){
      for(let i=0;i<14;i++){const date=addRosterDays(start,i);const header=dateWords.find(x=>x.d.getDate()===date.getDate()&&x.d.getMonth()===date.getMonth());if(!header)continue;const cx=(header.w.box.x0+header.w.box.x1)/2;const candidates=cleanWords.filter(w=>valid.has(w.token)&&Math.abs(((w.box.x0+w.box.x1)/2)-cx)<Math.max(35,(header.w.box.x1-header.w.box.x0)*1.3)&&Math.abs(((w.box.y0+w.box.y1)/2)-targetY)<80).sort((a,b)=>Math.abs(((a.box.y0+a.box.y1)/2)-targetY)-Math.abs(((b.box.y0+b.box.y1)/2)-targetY));if(candidates[0])assign[i]=candidates[0].token}
    }
    // Actuals/list roster: match each dated row with a valid code on the same y band.
    if(assign.filter(Boolean).length<3&&dateWords.length){
      dateWords.forEach(({w,d})=>{const idx=Math.round((new Date(d.getFullYear(),d.getMonth(),d.getDate())-new Date(start.getFullYear(),start.getMonth(),start.getDate()))/86400000);if(idx<0||idx>13)return;const cy=(w.box.y0+w.box.y1)/2;const candidates=cleanWords.filter(x=>valid.has(x.token)&&Math.abs(((x.box.y0+x.box.y1)/2)-cy)<24&&x.box.x0>w.box.x1).sort((x,y)=>x.box.x0-y.box.x0);if(candidates[0])assign[idx]=candidates[0].token})
    }
    return assign;
  }
  ...
  async function captureRosterScan(){
    const video=$('#rosterScanVideo'),canvas=$('#rosterScanCanvas'),status=$('#rosterScanStatus');if(!video?.videoWidth){toast('Camera is not ready');return}
    const maxW=1800,scale=Math.min(1,maxW/video.videoWidth);canvas.width=Math.round(video.videoWidth*scale);canvas.height=Math.round(video.videoHeight*scale);canvas.getContext('2d').drawImage(video,0,0,canvas.width,canvas.height);status.textContent='Reading roster…';
    try{
      const T=await loadTesseract();const result=await T.recognize(canvas,'eng',{logger:m=>{if(m.status==='recognizing text')status.textContent=`Reading roster… ${Math.round((m.progress||0)*100)}%`}});const data=result.data||{},words=data.words||[];const pe=detectPeriodEnd(data.text,words);
      if(!pe){status.textContent='Period End could not be detected. No shifts were changed. Reframe the full roster and scan again.';return}
      const assignments=scanAssignmentsFromWords(words,pe);pendingRosterScan={periodEnd:pe,assignments};closeRosterScanner();
      $('#rosterPeConfirmText').textContent=`Confirm PE: ${pe.toLocaleDateString('en-AU',{day:'numeric',month:'long',year:'numeric'})}`;$('#rosterPeSheet').hidden=false;
    }catch(err){status.textContent=err?.message||'Roster scan failed. No shifts were changed.'}
  }
