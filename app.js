...
(() => {
  ...
  function renderCalendar(selectedKey=''){
    ...
    for(let date=new Date(first);date<=last;date=addRosterDays(date,1)){
      ...
      button.innerHTML=`<span class="calendar-date">${date.getDate()}</span>
        <strong>${rosterDisplayCode(entry)}</strong><small>${displayRosterStart(row)||'&nbsp;'}</small>`;
  ...
  function buildRoster(){
    ...
    current.days.forEach((row,i)=>{
      ...
      card.querySelector('.offline-mode').onchange=()=>{
        card.dataset.entered='true';refreshShiftOptions(card,'');applyShiftDefaults(card,date,true);
  ...
  function renderPayResult(cycle){
    ...
    $('#payStatusLabel').textContent=statusLabel(status);$('#payRange').textContent=rangeLabel(cycle.startDate);
  ...
  function makePayCard(c){
    ...
    const card=document.createElement('div');card.className='saved-card compact-saved';
    card.innerHTML=`<button type="button" class="saved-card-toggle"><span class="saved-card-title"><span class="status-pill ${status}">${statusLabel(status)}</span><b>${rangeLabel(c.startDate)}</b></span><strong>${money(s.net)}</strong></button>
  ...
  async function shareRoster(){
    ...
    const lines=['BEGIN:VCALENDAR','VERSION:2.0','PRODID:-//PTA ShiftMate//Roster//EN','CALSCALE:GREGORIAN'];
    entries.forEach(e=>lines.push('BEGIN:VEVENT',`UID:pta-shiftmate-${e.key}@local`,`DTSTART;VALUE=DATE:${icsDate(e.date)}`,`DTEND;VALUE=DATE:${icsDate(addRosterDays(e.date,1))}`,`SUMMARY:Working - ${sharedShiftType(e.row)}`,'END:VEVENT'));
