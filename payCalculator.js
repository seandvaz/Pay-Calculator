...
window.PayCalc = (() => {
  ...
  const calculate = ({days,startDate,settings}) => {
    ...
    days.forEach((row,i)=>{
      ...
      if(data && st && fn && row.type!=='Off'){
        ...
        if(data.leaveType==='annual'){
          ...
          if(row.type!=='Leave'){
            if(data.allowance==='Morn/Aft'){
              const amount=parts.total*settings.maRate;
              extras+=amount;
              const startHour=Number((st||'0:00').split(':')[0])||0;
              allowanceDetails.push({
                type:startHour<12?'Morning allowance':'Afternoon allowance',
            ...
            if(data.allowance==='Night'){
              const amount=parts.total*settings.nightRate;
              extras+=amount;
              allowanceDetails.push({
                type:'Night allowance',
                date:date.toISOString().slice(0,10),
                dayIndex:i,
                code:row.code,
                hours:parts.total,
            ...
            if(row.code==='AA1'||row.hd){
              const amount=parts.total*settings.hdRate;
              extras+=amount;
              allowanceDetails.push({
                type:'Higher duties',
                date:date.toISOString().slice(0,10),
    ...
    return {gross,taxable,tax,hours,net,netHourly:hours?net/hours:0,dayTotals,breakdown,allowanceDetails};
  };
