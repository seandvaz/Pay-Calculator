...
window.PayCalc = (() => {
  ...
  const calculate = ({days,startDate,settings}) => {
    ...
    const breakdown={workedPay:0,annualLeavePay:0,sickLeavePay:0,lslPay:0,lwopPay:0,extrasPay:0};
    ...
    days.forEach((row,i)=>{
      ...
      if(data && st && fn && row.type!=='Off'){
        ...
        if(data.leaveType==='annual'){
          ...
          dayGross=baseComponent + baseComponent*(settings.annualLeaveLoading/100);
