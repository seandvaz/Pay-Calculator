window.PayCalc = (() => {
  ...
  const payrollRatesForDate=(date,settings)=>{
    ...
    if(key>='2026-05-22') return {maRate:4.87,earlyLateRate:5.77,annualLeaveLoadingRate:key>='2026-05-19'?12.5483:configuredLoading};
    if(key>='2026-05-19') return {maRate:4.66,earlyLateRate:5.53,annualLeaveLoadingRate:12.5483};
    if(key>='2025-10-07') return {maRate:4.66,earlyLateRate:5.53,annualLeaveLoadingRate:12.6188};
    return {maRate:4.48,earlyLateRate:5.31,annualLeaveLoadingRate:configuredLoading||11.6045};
  };
  ...
  const calculate=({days,startDate,settings})=>{
    ...
    rows.forEach(info=>{
      ...
      if(data&&data.leaveType){
        ...
        if(dayHoliday&&data.leaveType!=='lsl'&&data.leaveType!=='lwop'){
          ...
          const amount=leaveHours*settings.baseRate;dayGross+=amount;dayHours+=leaveHours;breakdown.sickLeavePay+=amount;
        ...
        const effectiveBase=settings.baseRate+(row.hd?settings.hdRate:0);
        ...
        if(nonPhHours>0&&row.type!=='Picked-up OT'){
          ...
          if(data.allowance==='Night'&&allowanceHours>0){
            const amount=allowanceHours*settings.nightRate;extras+=amount;allowanceDetails.push({type:'Night allowance',date:iso(date),dayIndex:i,code:row.code,hours:allowanceHours,amount});
          }
          const finishDate=addDays(date,mins(fn)<=mins(st)?1:0);
          const qualifyingEndpoint=(mins(st)>=61&&mins(st)<=239&&date.getDay()>=1&&date.getDay()<=5)||(mins(fn)>=61&&mins(fn)<=239&&finishDate.getDay()>=1&&finishDate.getDay()<=5);
      ...
      if(addHours>0&&data&&st&&fn){
        const effectiveBase=settings.baseRate+(row.hd?settings.hdRate:0),finishAbs=mins(st)+duration;
    ...
    rows.forEach((info,i)=>{const dayGross=info.prePhGross||0;gross+=dayGross;hours+=info.dayHours||0;dayTotals.push({gross:dayGross,hours:info.dayHours||0,start:info.st||'',finish:info.fn||'',additionalHours:Number(info.row.additionalHours)||0,additionalPay:info.additionalPay||0,holidayName:holidays.get(iso(info.date))||''})});
