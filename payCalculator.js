window.PayCalc = (() => {
  ...
  const calculate=({days,startDate,settings})=>{
    ...
    const addPhWork=(segment,rowInfo,isAdditional,effectiveBase)=>{
      ...
      const h=segment.minutes/60,benefit=rowInfo.row.phBenefit||'cash';bucket.hours+=h;
    ...
    rows.forEach(info=>{
      ...
      if(data&&data.leaveType){
        ...
        if(dayHoliday&&data.leaveType!=='lsl'&&data.leaveType!=='lwop'){
          ...
          const annualPart=Math.min(leaveHours,Math.max(0,Number(row.annualLeaveHours)||0));
          const sickPart=Math.max(0,leaveHours-annualPart);
        ...
        const segments=splitInterval(date,mins(st),duration);
        let nonPhHours=0,allowanceHours=0;
        segments.forEach(seg=>{
          const h=seg.minutes/60;dayHours+=h;
          if(addPhWork(seg,info,false,effectiveBase))return;
          nonPhHours+=h;const dow=seg.date.getDay();if(dow>=1&&dow<=5)allowanceHours+=h;let mult;
          if(row.type==='Picked-up OT')mult=(dow===0||dow===6)?settings.weekendOtMult:settings.addHoursMult;
          else mult=dow===6?settings.satMult:dow===0?settings.sunMult:settings.wdMult;
          const amount=h*effectiveBase*mult;baseComponent+=amount;dayGross+=amount;
        });
        breakdown.workedPay+=baseComponent;
        if(nonPhHours>0&&row.type!=='Picked-up OT'){
          if(data.allowance==='Morn/Aft'&&allowanceHours>0){
            const payableAllowanceHours=Math.ceil(allowanceHours-1e-9);
            const amount=payableAllowanceHours*payrollRatesForDate(date,settings).maRate;extras+=amount;allowanceDetails.push({type:(mins(st)<12*60?'Morning allowance':'Afternoon allowance'),date:iso(date),dayIndex:i,code:row.code,hours:payableAllowanceHours,amount});
          }
          ...
          const finishDate=addDays(date,mins(fn)<=mins(st)?1:0);
          const rosterStartIsWeekday=date.getDay()>=1&&date.getDay()<=5;
          const qualifyingEndpoint=rosterStartIsWeekday&&((mins(st)>=61&&mins(st)<=239)||(mins(fn)>=61&&mins(fn)<=239&&finishDate.getDay()>=1&&finishDate.getDay()<=5));
          ...
          if(row.offline&&row.offlineReason!=='cost-neutral'&&data.line!=='LEAVE'){
            const workedRoster=row.workedRosterLine||data.line;
            ...
            if(homeDesignated&&workedDesignated&&homeDesignated!==workedDesignated){
              ...
              const mileage=distance==null?0:distance*.895;
              const amount=travelPay+mileage;
              extras+=amount;
              allowanceDetails.push({
                type:'Other line allowance',date:iso(date),dayIndex:i,code:row.code,hours:1,amount,
                travelPay,mileage,distance,distanceMissing:distance==null,workedRosterLine:workedRoster
              });
            }
          }
      ...
      const earlyHours=Math.max(0,Number(row.earlyStartHours)||0);
      if(earlyHours>0&&data&&st&&fn){
        const effectiveBase=settings.baseRate+(row.hd?payrollRatesForDate(date,settings).hdRate:0);
        const rosterStart=dateAt(date,mins(st));
        const earlyStart=new Date(rosterStart.getTime()-earlyHours*3600000);
        const earlyDate=new Date(earlyStart.getFullYear(),earlyStart.getMonth(),earlyStart.getDate());
        const earlyStartMin=earlyStart.getHours()*60+earlyStart.getMinutes();
        splitInterval(earlyDate,earlyStartMin,earlyHours*60).forEach(seg=>{
          const h=seg.minutes/60;dayHours+=h;
          if(addPhWork(seg,info,true,effectiveBase))return;
          const dow=seg.date.getDay(),mult=(dow===0||dow===6)?settings.weekendOtMult:settings.addHoursMult,amount=h*effectiveBase*mult;
          additionalPay+=amount;dayGross+=amount;
        });
        breakdown.additionalHoursPay+=additionalPay;
      }

      const addHours=Math.max(0,Number(row.additionalHours)||0);
      if(addHours>0&&data&&st&&fn){
        const effectiveBase=settings.baseRate+(row.hd?payrollRatesForDate(date,settings).hdRate:0),finishAbs=mins(st)+duration;
        let extensionPay=0;
        splitInterval(date,finishAbs,addHours*60).forEach(seg=>{
          const h=seg.minutes/60;dayHours+=h;
          if(addPhWork(seg,info,true,effectiveBase))return;
          const dow=seg.date.getDay(),mult=(dow===0||dow===6)?settings.weekendOtMult:settings.addHoursMult,amount=h*effectiveBase*mult;
          extensionPay+=amount;additionalPay+=amount;dayGross+=amount;
        });
        breakdown.additionalHoursPay+=extensionPay;
      }
    ...
    rows.forEach((info,i)=>{const dayGross=info.prePhGross||0;gross+=dayGross;hours+=info.dayHours||0;dayTotals.push({gross:dayGross,hours:info.dayHours||0,start:info.st||'',finish:info.fn||'',earlyStartHours:Number(info.row.earlyStartHours)||0,additionalHours:Number(info.row.additionalHours)||0,additionalPay:info.additionalPay||0,holidayName:holidays.get(iso(info.date))||''})});
