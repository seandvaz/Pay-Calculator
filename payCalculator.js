    ...
    rows.forEach(info=>{
      ...
      if(data&&data.leaveType){
        ...
        if(dayHoliday&&data.leaveType!=='lsl'&&data.leaveType!=='lwop'){
          ...
          const amount=leaveHours*(settings.baseRate+settings.annualLeaveLoadingRate);dayGross+=amount;dayHours+=leaveHours;breakdown.annualLeavePay+=amount;
        ...
        if(nonPhHours>0&&row.type!=='Picked-up OT'){
          if(data.allowance==='Morn/Aft'&&allowanceHours>0){
            ...
            const amount=payableAllowanceHours*settings.maRate;extras+=amount;allowanceDetails.push({type:(mins(st)<12*60?'Morning allowance':'Afternoon allowance'),date:iso(date),dayIndex:i,code:row.code,hours:payableAllowanceHours,amount});
          }
          ...
          if(qualifyingEndpoint){
            const amount=settings.nightRate;extras+=amount;allowanceDetails.push({type:'Early/late shift allowance',date:iso(date),dayIndex:i,code:row.code,hours:1,amount});
          }
